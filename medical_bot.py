...
def confirm_missing_symptoms(user_symptoms, disease_symptoms):
    ...
    for symptom in list(missing_symptoms):
        if symptom not in confirmed_symptoms:  # Vérifie si le symptôme n'est pas déjà confirmé
            response = input(f"Do you have {symptom.replace('_', ' ')}? (yes/no or describe, type 'exit' to stop): ").strip().lower()

            if response == "exit":
                print("Exiting symptom confirmation.")
                break
    ...
    return confirmed_symptoms, rejected_symptoms

# Modifier la sauvegarde du rapport pour inclure les symptômes rejetés
def save_user_report(user_symptoms, confirmed_symptoms, rejected_symptoms, predicted_disease, disease_symptoms):
    report_filename = "user_reports/all_reports.txt"  # Fichier unique pour tous les rapports
...
# Amélioration 2 : Améliorer la précision de l'extraction des symptômes
...
# Le programme principal modifié pour inclure ces changements
if __name__ == "__main__":
    ...
    confirmed_symptoms, rejected_symptoms = confirm_missing_symptoms(symptoms_list, known_symptoms)
    ...
    if confirmed_symptoms < disease_symptoms:
        ...
        confirmed_symptoms = confirm_missing_symptoms(confirmed_symptoms, disease_symptoms)[0]

    print("The consultation is done for preconsultation")
    save_user_report(symptoms_list, confirmed_symptoms, rejected_symptoms, predicted_disease, disease_symptoms)
